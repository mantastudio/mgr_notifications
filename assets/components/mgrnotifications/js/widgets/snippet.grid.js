...
mgrnotifications.grid.Snippets = function (config) {
    config = config || {};
    ...
    Ext.applyIf(config, {
        ...
        ,viewConfig: {
            ...
            getRowClass: function (rec, ri, p) {
                var cls = 'mgrnotifications-row';

                if (this.showPreview) {
                    return cls + ' mgrnotifications-resource-expanded';
                }
                return cls + ' mgrnotifications-resource-collapsed';
            }
        }
        , tbar: [{
                text: 'Bulk Actions'
            ...
            ,{
                xtype: 'button'
                , id: 'mgrnotifications-snippets-reload'
                , text: 'reload'
                , listeners: {
    ...
    mgrnotifications.grid.Snippets.superclass.constructor.call(this, config)
};
Ext.extend(mgrnotifications.grid.Snippets, MODx.grid.Grid, {
    ...
    , changeCategory: function (btn, e) {
        ...
        if (!this.changeCategoryWindow) {
            this.changeCategoryWindow = MODx.load({
                  xtype: 'mgrnotifications-snippet-window-change-category'
                  , record: r
                  , listeners: {
                    'success': {fn: function (r) {
                        // this.refresh();
                        var sels = this.getSelectionModel().getSelections();
                        var cat = Ext.getCmp('mgrnotifications-snippet-category-combo').lastSelectionText;
                        var s = this.getStore();
    ...
    , snippetRemove: function () {
        ...
        if (cs === false) return false;
        MODx.msg.confirm({
             title: _('mgrnotifications.delete')
             , text: _('mgrnotifications.confirm_delete')
             , url: this.config.url
             , params: {
...
Ext.reg('mgrnotifications-grid-snippet', mgrnotifications.grid.Snippets);


mgrnotifications.window.ChangeCategory = function (config) {
    config = config || {};
    Ext.applyIf(config, {
        ...
        ,fields: [{
            ...
            ,id: 'mgrnotifications-snippet-category-combo'
    ...
    mgrnotifications.window.ChangeCategory.superclass.constructor.call(this, config);
};
Ext.extend(mgrnotifications.window.ChangeCategory, MODx.Window);
Ext.reg('mgrnotifications-snippet-window-change-category', mgrnotifications.window.ChangeCategory);
